...
// MmapFile represents an mmapd osFile and includes both the buffer to the data and the osFile descriptor.
...
func OpenMmapFileUsing(fd *os.File, sz int, writable bool) (*MmapFile, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "cannot stat osFile: %s", filename)
	}
	...
	buf, err := mmap.Mmap(fd, writable, fileSize) // Mmap up to osFile size.
	if err != nil {
...
// OpenMmapFile opens an existing osFile or creates a new osFile. If the osFile is
// created, it would truncate the osFile to maxSz. In both cases, it would mmap
// the osFile to maxSz and returned it. In case the osFile is created, z.NewFile is
// returned.
func OpenMmapFile(filename string, flag int, maxSz int) (*MmapFile, error) {
	// fmt.Printf("opening osFile %s with flag: %v\n", filename, flag)
...
func (m *MmapFile) AllocateSlice(sz, offset int) ([]byte, int, error) {
	...
	// If the osFile is too small, double its size or increase it by 1GB, whichever is smaller.
...
func (m *MmapFile) Delete() error {
	...
	if err := mmap.Munmap(m.Data); err != nil {
		return fmt.Errorf("while munmap osFile: %s, error: %v\n", m.Fd.Name(), err)
	}
	m.Data = nil
	if err := m.Fd.Truncate(0); err != nil {
		return fmt.Errorf("while truncate osFile: %s, error: %v\n", m.Fd.Name(), err)
	}
	if err := m.Fd.Close(); err != nil {
		return fmt.Errorf("while close osFile: %s, error: %v\n", m.Fd.Name(), err)
	}
	return os.Remove(m.Fd.Name())
}

// Close would close the osFile. It would also truncate the osFile if maxSz >= 0.
func (m *MmapFile) Close() error {
	if m.Fd == nil {
		return nil
	}
	if err := m.Sync(); err != nil {
		return fmt.Errorf("while sync osFile: %s, error: %v\n", m.Fd.Name(), err)
	}
	if err := mmap.Munmap(m.Data); err != nil {
		return fmt.Errorf("while munmap osFile: %s, error: %v\n", m.Fd.Name(), err)
	}
