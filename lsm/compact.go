...
func (lm *levelManager) runCompacter(id int) {
	...
	randomDelay := time.NewTimer(time.Duration(rand.Int31n(1000)) * time.Millisecond) //生成一个随机延迟的时间
	select {
	case <-randomDelay.C:
	...
	ticker := time.NewTicker(50 * time.Millisecond) //周期性地进行compact
	defer ticker.Stop()
	for {
		select {
...
func (lm *levelManager) runOnce(id int) bool {
	prios := lm.pickCompactLevels() //选择参与压缩的层
	if id == 0 {
		// 0号协程 总是倾向于压缩L0层，即对L0层提权
...
func moveL0toFront(prios []compactionPriority) []compactionPriority {
	...
	//如果L0层在优先级数组里但不是位于开头，那就把它直接移动到开头
	if idx > 0 {
...
func (lm *levelManager) doCompact(id int, p compactionPriority) error {
	...
	if p.t.baseLevel == 0 {
		p.t = lm.levelTargets() //再去重新选择一遍目标Level
	}
	// 创建真正的压缩计划
	cd := compactDef{
		compactorId:  id,
...
// levelTargets 计算得到要压缩到的目标层
func (lm *levelManager) levelTargets() targets {
	//这是一个函数闭包，会在后续计算中用到
	adjust := func(sz int64) int64 {
		if sz < lm.opt.BaseLevelSize {
			return lm.opt.BaseLevelSize //配置中对于一整个Level的预期size
		}
		return sz
	}
	...
	// 为了减少写放大，从最后一个level开始计算
	dbSize := lm.lastLevel().getTotalSize() //获取当前LSM Tree最后一层的实际大小
	for i := len(lm.levels) - 1; i > 0; i-- {
		leveTargetSize := adjust(dbSize)
		t.targetSz[i] = leveTargetSize //更新每一层的目标size
		// 如果当前的level没有达到合并的要求
		if t.baseLevel == 0 && leveTargetSize <= lm.opt.BaseLevelSize { //只要t.baseLevel更新过一次就不会再进入这个循环里
			t.baseLevel = i
		}
		dbSize /= int64(lm.opt.LevelSizeMultiplier) //维护上层和下层的size所遵循的一个数量级关系
	}

	tsz := lm.opt.BaseTableSize //配置中对于某个Level中一个sst文件的预期size
...
func (lm *levelManager) fillTablesL0ToLbase(cd *compactDef) bool {
	...
	if len(cd.bot) == 0 {
		cd.nextRange = cd.thisRange //如果目标层没有可以参与压缩的sst，那就在L0层内进行压缩
	} else {
		cd.nextRange = getKeyRange(cd.bot...)
	}
	return lm.compactState.compareAndAdd(thisAndNextLevelRLocked{}, *cd)
...
func (lm *levelManager) subcompact(it utils.Iterator, kr keyRange, cd compactDef,
	...
	for it.Valid() {
		...
		go func(builder *tableBuilder) {
			...
			sstName := utils.SSTableFullPath(lm.opt.WorkDir, newFID)
...
//原子地更新compact全局状态表，避免和其他压缩计划涉及到相同的sst文件进而产生冲突
func (cs *compactStatus) compareAndAdd(_ thisAndNextLevelRLocked, cd compactDef) bool {
	...
	thisLevel.delSize += cd.thisSize
	for _, t := range append(cd.top, cd.bot...) {
		cs.tables[t.fid] = struct{}{} //tables的作用是记录当前处于压缩状态的sst文件有哪些
	}
	return true
}
